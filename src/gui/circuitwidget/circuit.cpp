...
void Circuit::loadStrDoc( QString &doc )
{
    ...
    QList<QStringView> docLines = QStringView{doc}.split(QString("\n"));
    // for( QStringRef line : docLines )
    for( QStringView line : docLines )
    {
        if( line.startsWith( QString("<circuit")) && !m_pasting )
        {
            line = line.mid( 9, line.length()-11 );
            //QVector<QStringRef> props = line.split(" ");
            QVector<QStringView> props = QStringView(line).split(QString(" "));
            // for( QStringRef prop : props )
            for( QStringView prop : props )
            {
                // QVector<QStringRef> p = prop.split("=");
                QVector<QStringView> p = QStringView(prop).split(QString("="));
                if( p.size() != 2 ) continue;

                // QStringRef name = p.first(), val = p.last();
                QStringView name = p.first(), val = p.last();
                val = val.mid( 1, val.lastIndexOf(QString("\""))-1 ); // Remove "

                if     ( name == QString("stepSize") ) m_simulator->setStepSize( val.toULongLong() );
                else if( name == QString("stepsPS") ) m_simulator->setStepsPerSec( val.toULongLong() );
                else if( name == QString("NLsteps") ) m_simulator->setMaxNlSteps( val.toUInt() );
                else if( name == QString("reaStep") ) m_simulator->setreactStep( val.toULongLong() );
                else if( name == QString("animate") ) setAnimate( val.toInt() );
            }
        }
        else if( line.contains(QString("<mainCompProps")) )
        {
            ...
            QString propName = "";
            // QVector<QStringRef> props = line.split("\"");
            QList<QStringView> props = line.split(QString("\""));
            // for( QStringRef prop : props )
            for( QStringView prop : props )
            {
                if( prop.endsWith( QString("=")) )
                {
                    prop = prop.split(QString(" ")).last();
                    propName = prop.toString().mid( 0, prop.length()-1 );
                    continue;
                }
                else if( prop.endsWith(QString("/>")) ) continue;
        ...
        else if( line.startsWith(QString("<item")) )
        {
            ...
            for( QStringView prop : props )
            {
                if( prop.size() > 1 && prop.endsWith(QString("=")) )
                {
                    prop = prop.split(QString(" ")).last();
                    ...
                    if     ( name == QString("itemtype")   ) type  = prop.toString();
                    else if( name == QString("CircId")     ) uid   = prop.toString();
                    else if( name == QString("objectName") ) uid   = prop.toString();
                    else if( name == QString("label")      ) label = prop.toString();
                    else if( name == QString("id")         ) label = prop.toString();
            ...
            {
                if( type == QString("Subcircuit") || type == QString("MCU") )
                     newUid = uid.split(QString("-")).first()+"-"+newSceneId();
                else newUid = type+"-"+newSceneId();
            }
            else     newUid = uid;

            if( type == QString("Connector") )
            {
                Pin* startpin = NULL;
                Pin* endpin   = NULL;
                ...
                for( QStringView prop : properties )
                {
                    ...
                    if     ( name == QString("startpinid")) startpinid = prop.toString();
                    else if( name == QString("endpinid")  ) endpinid   = prop.toString();
                    else if( name == QString("pointList") ) pointList  = prop.toString().split(QString(","));
                    else if( name == QString("uid")       ) uid   = prop.toString();
                    name = QString("");
                }
                if( m_pasting )
                {
            ...
            else if( type == "Node")
            {
                ...
                {
                    if( name.isEmpty() ) { name = prop.toString(); continue; }
                    if     ( name == QString("Pos")) joint->setPropStr( "Pos", prop.toString() );
                    else if( name == QString("x")  ) joint->setX( prop.toInt() );
                    else if( name == QString("y")  ) joint->setY( prop.toInt() );
                    name = "";
                }
                int number = joint->objectName().split(QString("-")).last().toInt();
                if( number > m_seqNumber ) m_seqNumber = number; // Adjust item counter: m_seqNumber
                addItem( joint );
                nodeList.append( joint );
            }
            else if( type == QString("Plotter") ) ;   // Old Plotter widget
            else if( type == QString("SerialTerm")) ; // Old SerialTerm
            else{
                bool oldArduino = false;
                if( type == QString("Arduino") ){
                    oldArduino = true;
                    type = QString("Subcircuit");
...
void Circuit::importCirc(  QPointF  )
{
    if( m_conStarted ) return;
    m_pasting = true;
...
void Circuit::drawBackground( QPainter*  painter, const QRectF & )
{
