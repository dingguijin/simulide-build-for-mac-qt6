...
void Pin::setLabelPos()
{
    ...
    if( m_angle == 0 )         // Right side
    {
        ...
        else                xlabelpos -= fm.boundingRect(m_label.text()).width()+offset;
    ...
    else if( m_angle == 90 )   // Top
    {
        ...
        if( m_Vflip == -1 ) ylabelpos += fm.boundingRect(m_label.text()).width()+offset;
    ...
    else if( m_angle == 180 )  // Left
    {
        ...
        if( m_Hflip == -1 ) xlabelpos += fm.boundingRect(m_label.text()).width()+offset;
...
int Pin::labelSizeX()
{
    ...
    return fm.boundingRect( m_label.text() ).width();
}
