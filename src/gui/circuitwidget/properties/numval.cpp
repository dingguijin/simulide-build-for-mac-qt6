...
void NumVal::setup()
{
    ...
    if( unit.isEmpty() || unit.startsWith("_") )
    {
        ...
        int index = unit.indexOf(r, 0, &match);
        if( index == 0 ) un.remove( 0, 1 ); // Remove multiplier
...
void NumVal::on_valueBox_valueChanged( double )
{
    if( m_blocked ) return;
    m_blocked = true;
...
void NumVal::on_unitBox_currentTextChanged( QString )
{
    if( m_blocked ) return;
    m_blocked = true;
