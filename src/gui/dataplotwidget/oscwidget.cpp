...
void OscWidget::on_channelGroup_buttonClicked( QAbstractButton* btn )
{
    m_channel = channelGroup->id(btn);
    ...
    if( m_channel < 4 ){
        updateTimePosBox( m_channel, m_oscope->timePos( m_channel ) );
        updateVoltDivBox( m_channel, m_oscope->voltDiv( m_channel ) );
        updateVoltPosBox( m_channel, m_oscope->voltPos( m_channel ) );
}    }

void OscWidget::on_autoGroup_buttonClicked( QAbstractButton* btn )
{
    m_oscope->setAutoSC( autoGroup->id(btn) ); 
}
...
void OscWidget::on_triggerGroup_buttonClicked( QAbstractButton* btn )
{
    m_oscope->setTrigger( triggerGroup->id(btn) ); 
}
...
void OscWidget::on_hideGroup_buttonClicked( QAbstractButton* btn )
{
    int ch = hideGroup->id(btn);
    if( ch == 4 )
    {
...
void OscWidget::on_trackGroup_buttonClicked( QAbstractButton* btn) {
    m_oscope->setTracks( trackGroup->id(btn) );
}
...
void OscWidget::mousePressEvent( QMouseEvent* event )
{
    ...
    m_mousePos = event->globalPosition().x();
...
void OscWidget::mouseMoveEvent( QMouseEvent* event )
{
    ...
    int pos = event->globalPosition().x();
    if( m_action == actMove )
    {
