...
int CodeEditor::getsyntaxCoincidences()
{
    ...
    for( QString line : lines )
    {
        ...
        { if( line.contains( QRegularExpression( "\\b"+instruction+"\\b" ) )) avr++; }
        for( QString instruction : m_picInstr )
        { if( line.contains( QRegularExpression( "\\b"+instruction+"\\b" ) )) pic++; }
        for( QString instruction : m_i51Instr )
        { if( line.contains( QRegularExpression( "\\b"+instruction+"\\b" ) )) i51++; }
        if( ++nlines > 200 ) break;
    }
    int result = 0;
...
void CodeEditor::setTabSize( int size )
{
    ...
    for( CodeEditor* doc : m_documents )
    {
        // doc->setTabStopDistance( m_tabSize*m_fontSize*2/3 );
        doc->setTabStopDistance( m_tabSize*m_fontSize*2/3 );
...
int CodeEditor::lineNumberAreaWidth()
{
    ...
    return  fontMetrics().height() + fontMetrics().boundingRect( QLatin1Char( '9' ) ).width() * digits;
}
