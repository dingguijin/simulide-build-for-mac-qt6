			...
			if(pThirdOp < end && jumpTable[pThirdOp - start] == nullptr) {
				...
				case asBC_SetV8:
					if(thirdOp == asBC_CpyVtoV8 &&
                        ...
                        fpu.load_double(*edi-offset(pNextOp,1)); //Optimize <Variable Double> <op>= <Constant Double>
...
#define pushPrep(use) \
	...
	reservedPushBytes -= use;
        ...
        case asBC_JZ:  bl &= *&bl; do_jump(Zero); break;
		case asBC_JLowNZ:
        case asBC_JNZ: bl &= *&bl; do_jump(NotZero); break;
        case asBC_JS:  bl &= *&bl; do_jump(Sign); break;
        case asBC_JNS: bl &= *&bl; do_jump(NotSign); break;
        case asBC_JP:  bl == 0; do_jump(Greater); break;
        case asBC_JNP: bl == 0; do_jump(LessOrEqual); break;

        case asBC_TZ:  bl &= *&bl; ebx.setIf(Zero); ebx.copy_zeroing(ebx); break;
        case asBC_TNZ: bl &= *&bl; ebx.setIf(NotZero); ebx.copy_zeroing(ebx); break;
        case asBC_TS:  bl &= *&bl; ebx.setIf(Sign); ebx.copy_zeroing(ebx); break;
        case asBC_TNS: bl &= *&bl; ebx.setIf(NotSign); ebx.copy_zeroing(ebx); break;
        case asBC_TP:  bl == 0; ebx.setIf(Greater); ebx.copy_zeroing(ebx); break;
        case asBC_TNP: bl == 0; ebx.setIf(LessOrEqual); ebx.copy_zeroing(ebx); break;
...
void SystemCall::call_entry(asSSystemFunctionInterface* , asCScriptFunction* sFunc) {
	unsigned pBits = sizeof(void*) * 8;
#ifdef JIT_64
...
void SystemCall::call_exit(asSSystemFunctionInterface* ) {
